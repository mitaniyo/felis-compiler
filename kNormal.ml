...
let rec fv = function (* 式に出現する（自由な）変数 (caml2html: knormal_fv) *)
  ...
  | Neg(x) | FNeg(x) | FAbs(x) | Sqrt(x) -> S.singleton x
...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax.Var(x) -> (* 外部配列の参照 (caml2html: knormal_extarray) *)
      ...
      | Type.Tuple(tl) as t-> ExtTuple(x), t
      | t -> print_type t; failwith (Printf.sprintf "external variable %s does not have an array type" x))
